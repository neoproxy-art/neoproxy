...
const RINGS = [
  { radius: 3,  label: 'KICK',  color: '#ff3366', freq: 60,  type: 'sine',    gain: 0.8 },
  { radius: 5,  label: 'BASS',  color: '#ff9500', freq: 120, type: 'triangle', gain: 0.6 },
  { radius: 7,  label: 'SYNTH', color: '#00d4ff', freq: 440, type: 'square',   gain: 0.3 },
  { radius: 9,  label: 'HI',    color: '#00ff9d', freq: 880, type: 'sawtooth', gain: 0.2 },
]
...
export default function DJPage() {
  ...
  const playSound = useCallback((ring: typeof RINGS[0]) => {
    ...
    osc.type = ring.type as OscillatorType
    osc.frequency.setValueAtTime(ring.freq, ctx.currentTime)
    gain.gain.setValueAtTime(ring.gain, ctx.currentTime)
  ...
  const spawnParticle = useCallback((shape: ShapeType) => {
    ...
    switch (shape) {
      case 'box':
        mesh = BABYLON.MeshBuilder.CreateBox('p'+particleIdRef.current, { size: 0.3 }, scene)
        break
      case 'cylinder':
        mesh = BABYLON.MeshBuilder.CreateCylinder('p'+particleIdRef.current, { height: 0.4, diameter: 0.25 }, scene)
        break
      case 'torus':
        mesh = BABYLON.MeshBuilder.CreateTorus('p'+particleIdRef.current, { diameter: 0.4, thickness: 0.1 }, scene)
        break
      default:
        mesh = BABYLON.MeshBuilder.CreateSphere('p'+particleIdRef.current, { diameter: 0.3 }, scene)
    }
    const mat = new BABYLON.PBRMaterial('pm'+particleIdRef.current, scene)
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      engine.runRenderLoop(() => {
        ...
        // Rotar anillos levemente
        rings.forEach((r, i) => {
          r.rotation.y += 0.002 * (i % 2 === 0 ? 1 : -1)
        })
        ...
        particlesRef.current.forEach((p, idx) => {
          ...
          RINGS.forEach((ring, ri) => {
            ...
            if (delta < 0.25) {
              ...
              if (rm) {
                rm.mat.emissiveColor = BABYLON.Color3.FromHexString(ring.color).scale(1.5)
                setTimeout(() => {
                  rm.mat.emissiveColor = BABYLON.Color3.FromHexString(ring.color).scale(0.3)
                }, 150)
              }
              // Efecto de impacto
              ...
              setTimeout(() => {
                if (p.mesh.material) {
                  p.mesh.material.emissiveColor = new BABYLON.Color3(0.8, 0.9, 1)
                }
              }, 100)
            }
          })
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', background: '#00000d', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', top: 20, left: 28, pointerEvents: 'none' }}>
        <div style={{ fontFamily: '"Space Mono",monospace', fontSize: 9, color: 'rgba(0,212,255,0.4)', letterSpacing: '0.2em', marginBottom: 4 }}>
          // NEOPROXY · DJ MODULE
        </div>
        <div style={{ fontFamily: '"Space Mono",monospace', fontSize: 22, fontWeight: 700, color: '#fff', letterSpacing: '0.1em' }}>
          GEO<span style={{ color: '#b400ff' }}>SYNC</span>
        </div>
      </div>
        ...
        {RINGS.map(r => (
          <div key={r.label} style={{
            ...
            <div style={{
              ...
              background: activeRings.includes(r.label) ? r.color : 'transparent',
              border: `1px solid ${r.color}`,
              boxShadow: activeRings.includes(r.label) ? `0 0 8px ${r.color}` : 'none',
              transition: 'all 0.1s',
            }}/>
            <span style={{ color: activeRings.includes(r.label) ? r.color : 'rgba(255,255,255,0.3)', letterSpacing: '0.1em' }}>
              {r.label}
            </span>
          </div>
        ))}
      ...
      <div style={{
        ...
        <button
          onClick={togglePlay}
          style={{
            background: playing ? 'rgba(180,0,255,0.2)' : 'rgba(0,212,255,0.1)',
            border: `1px solid ${playing ? '#b400ff' : '#00d4ff'}`,
            color: playing ? '#b400ff' : '#00d4ff',
            fontFamily: '"Space Mono",monospace',
          ...
          <input
            ...
            onChange={e => {
              ...
              if (playing && intervalRef.current) {
                ...
                const interval = (60 / Number(e.target.value)) * 1000 / 2
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {SHAPES.map(sh => (
            <button
              ...
              style={{
                background: selectedShape === sh ? 'rgba(0,212,255,0.15)' : 'transparent',
                border: `1px solid ${selectedShape === sh ? '#00d4ff' : 'rgba(0,212,255,0.2)'}`,
                color: selectedShape === sh ? '#00d4ff' : 'rgba(255,255,255,0.3)',
                fontFamily: '"Space Mono",monospace',
